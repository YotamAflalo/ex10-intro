...
class Apples:
    ...
    def apple_wall_collision(self,wall_locs:dict):
        ...
        :param wall_locs: dict of walls locations (as list of [row,col])
        :return: None
        '''
        apple_locs = self.ap_locs.copy()
        for ap_loc in apple_locs:
            for wall in wall_locs.keys():
                if list(ap_loc) in wall_locs[wall]:
    ...
    def wall_generetor(self, ap_locs: set, snake_locs: set):
        ...
        for i in range(-1,2,1): #generate wall with 3 bricks
            ...
            for key in self.walls_loc.keys():
                if list(new_loc) in self.walls_loc[key]:
                    return None

        #making list with all the brick places
        place_list = [[x_wall + j * key_change[dir_wall][0],
                       y_wall + j * key_change[dir_wall][1]] for j in range(-1, 2, 1)]
        self.walls_loc[(x_wall,y_wall,dir_wall)] = place_list #place the new wall
