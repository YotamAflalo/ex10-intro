...
class Walls:
    ...
    def wall_generetor(self,ap_locs:set=None,snake_locs:set=None):
    ...
    def wall_cleaner(self):
        pass #צריך לבנות פונקציה שמוחקת לוחות שיצאו מהמסך. דורש קריאה לארגז
...
for i in range(4):
    if walls.need_more_walls():
        walls.wall_generetor()
        print(walls.walls_loc)
for i in range(6):
    walls.wall_move()
    print(walls.walls_loc)

for i in range(2):
    walls.wall_remove(list(walls.walls_loc.keys())[0])
    print(walls.walls_loc)'''
